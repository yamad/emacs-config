...
(defvar yacc--declarers '("%union" "%token" "%type"
			   "%left" "%right" "%nonassoc")
  "commands which can declare a token or state type")

(defvar yacc-font-lock-keywords-1 c-font-lock-keywords
  "Basic highlighting for Yacc mode.")

(defvar yacc-font-lock-keywords-2
  (append
   (list
    (cons (concat "^\\(" (regexp-opt yacc--declarers) "\\)")
	  '(1 font-lock-keyword-face))
    )
   yacc-font-lock-keywords-1)
  "Gaudy highlighting for Yacc mode.")

(defvar yacc-font-lock-keywords yacc-font-lock-keywords-2
  "Default expressions to highlight in Yacc mode")
...
; doesn't seem to be executed?!  Twice gets it set right.
  ...
  (setq font-lock-defaults '((yacc-font-lock-keywords
			      yacc-font-lock-keywords-1
			      yacc-font-lock-keywords-2)
			     nil nil nil))
  (save-excursion
    (goto-char (point-min))
    (let ((section 1))
